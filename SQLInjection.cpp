...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  //  something more generic
    ...
    //Maybe we should consider a whitelisting approach instead?
    //const char* injectionBlacklistRule = "(--|\\/\\*|\\*\\/|=|<|>|SELECT|DROP|\\(|\\))";
    //std::regex blacklistPattern(injectionBlacklistRule);
    // //regex_search returns true if a substring of the string matches the rule.
    //if (std::regex_search(sql, blacklistPattern)) {
    //    std::cout << "Suspected injection attempt detected" << std::endl;
    //    return false;
    //}
    ...
    const char* injectionWhitelistRule = "[A-Za-z0-9_\\.]";
    std::regex whitelistPattern(injectionWhitelistRule);
    //regex_match returns false if the entire string does NOT match the rule.
    if (!std::regex_match(sql, whitelistPattern)) {
        std::cout << "Suspected injection attempt detected" << std::endl;
        return false;
    }
  ...
  //using parameterized query on param sql
