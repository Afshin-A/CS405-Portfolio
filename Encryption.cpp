...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
  ...
  //  Line 4+: data
    ...
    //default for open-mode is std::ios::in
    std::fstream fileWriter(filename, std::ios::out);
    try {
        if (fileWriter.is_open()) {
            fileWriter << student_name << '\n';
            fileWriter << currentTime << '\n';
            fileWriter << key << '\n';
            fileWriter << data << '\n';
            fileWriter.close();
        }
...
int main()
{
  ...
  const std::string encrypted_string = encrypt_decrypt(student_name, key);
